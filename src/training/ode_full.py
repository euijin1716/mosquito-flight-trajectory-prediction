"""
ode_full.py (구 train_phaseODE_full.py) — Neural ODE 전체 데이터 학습 (홀드아웃 없음, 제출용)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Neural ODE(댐핑 가속도장 RK4 적분)를 전체 10,000개(=50,000 examples) 캐시로 재학습.
- 홀드아웃 없음 → EMA 가중치를 최종 채택 (일반화 booster)
- --seed 로 시드별 1개 학습 → goh30에서 10시드 앙상블 + Y-flip TTA

실행: PREFIX=ode_full RW=2.0 RT=0.0015 python -m src.training.ode_full --seed 0
결과: models/ode_full_{0~4}.pt  (EMA 가중치)
"""
...
from src.training import ode_oof as tc

MODEL = ROOT/'models'
import os as _os
PREFIX=_os.environ.get('PREFIX','ode_full')   # 기본값 교정: 구버전엔 GRU 이름(복사 흔적)이라 미지정 실행 시 GRU 가중치를 덮어쓸 수 있었음
FULL_EPOCHS = 55
N_SEEDS = 5
EMA_DECAY = 0.9
EMA_START = 8
...
def main():
    ...
    print(f'Neural ODE 전체학습 | {FULL_EPOCHS}ep × {N_SEEDS}seeds (EMA) | device={tc.get_device()}')
    t0 = time.time()
    ...
    print(f'전체 {(time.time()-t0)/60:.1f}분 | 모델 ode_full_0~{N_SEEDS-1}.pt')
