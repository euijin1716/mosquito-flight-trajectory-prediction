"""
gru_oof.py (구 train_phaseG.py) — GRU 5-fold OOF 학습 + 내부전이 캐시 빌드
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
실행: python -m src.training.gru_oof
"""
...
class MaskedBiGRU(nn.Module):
    """masked-mean + last + attention pooling 요약."""
    def __init__(self,seq_dim=13,scal_dim=22,h=HID,nl=NL,dr=DR):
        super().__init__()
        self.proj=nn.Sequential(nn.Linear(seq_dim,h),nn.LayerNorm(h))
        self.enc_tcn = (ENC=='tcn')
        if self.enc_tcn:
            ch=h*2
            self.tcn=nn.ModuleList([nn.Conv1d(h if i==0 else ch, ch, 3, padding=d, dilation=d) for i,d in enumerate([1,2,4,1])])
            self.tcn_ln=nn.ModuleList([nn.GroupNorm(1,ch) for _ in range(4)])
            self.tcn_do=nn.Dropout(dr)
        else:
            self.gru=nn.GRU(h,h,nl,batch_first=True,bidirectional=True,dropout=dr if nl>1 else 0)
        self.attn=nn.Linear(h*2,1)                       # attention scoring
        hw=HEADW; ctx_dim=(h*6 if POOL=='attn' else h*4)+scal_dim
        self.head=nn.Sequential(nn.Linear(ctx_dim,hw),nn.GELU(),nn.Dropout(dr),
                                nn.Linear(hw,hw//4),nn.GELU(),nn.Linear(hw//4,3))
    def forward(self,seq,scal,mask):
        x=self.proj(seq)                                # (B,11,h)
        if self.enc_tcn:
            z=x.transpose(1,2)                          # (B,h,11)
            for conv,ln in zip(self.tcn,self.tcn_ln):
                z=self.tcn_do(torch.nn.functional.gelu(ln(conv(z))))
            out=z.transpose(1,2)                         # (B,11,2h)
        else:
            out,_=self.gru(x)                            # (B,11,2h)
        last=out[:,-1,:]                                 # 좌측패딩이라 마지막=유효
        m=mask.unsqueeze(-1)
        mean=(out*m).sum(1)/m.sum(1).clamp(min=1)        # masked mean
        if POOL=='attn':
            score=self.attn(out).squeeze(-1).masked_fill(mask<0.5,-1e9)
            att=(torch.softmax(score,dim=1).unsqueeze(-1)*out).sum(1)
            ctx=torch.cat([last,mean,att,scal],-1)
        else:
            ctx=torch.cat([last,mean,scal],-1)
        return self.head(ctx)

def run_fold(fold):
    dev=get_device(); log=get_logger(f'gru_{TAG}_{fold}',EXP/f'log_gru_{fold}.txt')
    ...
    best=0.0; bep=0; pat=0; path=MODEL/f'gru_{TAG}_fold_{fold}.pt'; t0=time.time()
...
def merge():
    ...
    for fold,(_,va_traj) in enumerate(splits):
        ...
        ck=torch.load(MODEL/f'gru_{TAG}_fold_{fold}.pt',map_location=dev,weights_only=False); frh.append(ck['val_rhit'])
    ...
    print(f'GRU OOF R-Hit = {orh:.4f}')
    for lab,rh in speed_bin_rhit(oof,labels,_spd_real(d)).items():
        print(f'  {lab:>8} : {rh:.4f}')
    np.savez(EXP/f'gru_{TAG}_oof.npz',preds=oof,true=labels,speeds=_spd_real(d))
    print(f'저장: gru_{TAG}_oof.npz')
